...
$reportname = get_string('reportname', 'block_inactiveuseralert');
$title = format_string($course->fullname) . ": $reportname";

$pageurl = new moodle_url('/blocks/inactiveuseralert/index.php', array('course' => $courseid));
if ($alertid) {
...
$renderer = $PAGE->get_renderer('block_inactiveuseralert');
$report = new \block_inactiveuseralert\report();
$alertlist = \block_inactiveuseralert\helper::get_alerts($courseid);
$selected = $alertid;
...
$sentcount = !empty($alertid) ? \block_inactiveuseralert\helper::load_alerts_with_track($alertid) : [];
$renderable = new \block_inactiveuseralert\output\report\renderable($name, $userdata, $count, $report->page,
